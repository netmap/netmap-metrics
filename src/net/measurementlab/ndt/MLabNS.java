...
public class MLabNS {
  ...
  static public String Lookup(Context context, String tool, String address_family, String metro) {
    ...
    try {
      ...
      Log.d(LOG_TAG, "Creating request GET for mlab-ns"+ address_family+ "," + metro);
      ...
      Log.e(LOG_TAG, "SocketTimeoutException trying to contact m-lab-ns");
      // e.getMessage() is null       
      throw new InvalidParameterException("Connect to m-lab-ns timeout. Please try again.");
    } catch (IOException e) {
      Log.e(LOG_TAG, "IOException trying to contact m-lab-ns: " + e.getMessage());
      throw new InvalidParameterException(e.getMessage());
    } catch (JSONException e) {
      Log.e(LOG_TAG, "JSONException trying to contact m-lab-ns: " + e.getMessage());
      throw new InvalidParameterException(e.getMessage());
    } finally {
      if (inputStream != null) {
        try {
          ...
          Log.e(LOG_TAG, "Failed to close the input stream from the HTTP response");
        }
      }
    }
  }
  ...
  public static String prepareUserAgent(Context context) {
    // Hardcoded User-Agent to reflect the library using this code.
    return "NetMap/1.0 Recorder/1.0";
  }
